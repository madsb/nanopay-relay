...
CREATE TABLE offers (
  ...
  CONSTRAINT offers_pricing_check CHECK (
    (pricing_mode = 'fixed' AND fixed_price_raw IS NOT NULL)
    OR (pricing_mode = 'quote' AND fixed_price_raw IS NULL)
  )
);

CREATE INDEX idx_offers_seller_pubkey ON offers (seller_pubkey);
CREATE INDEX idx_offers_active ON offers (active);
CREATE INDEX idx_offers_pricing_mode ON offers (pricing_mode);
CREATE INDEX idx_offers_created_at_desc ON offers (created_at DESC);
...
CREATE INDEX idx_jobs_seller_status_updated ON jobs (seller_pubkey, status, updated_at DESC);
CREATE INDEX idx_jobs_buyer_status_updated ON jobs (buyer_pubkey, status, updated_at DESC);
CREATE INDEX idx_jobs_status ON jobs (status);
CREATE INDEX idx_jobs_quote_expires_at ON jobs (quote_expires_at) WHERE status = 'quoted';
CREATE INDEX idx_jobs_lock_expires_at ON jobs (lock_expires_at) WHERE lock_owner IS NOT NULL;
